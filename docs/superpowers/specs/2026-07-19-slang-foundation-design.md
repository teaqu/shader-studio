...
Existing shaders retain legacy behavior. Shader Studio explicitly compiles a root shader as `#language slang legacy` when it has no language directive, rather than relying on the bundled compiler's changing default. This fallback preserves the behavior of every existing `.slang` file.
...
New `.slang` files generated by Shader Studio use the newest pinned, non-`latest` language version supported by the bundled compiler. For the compiler pinned by this design, the generated header is:
...
This new-file default changes only generated source; opening an existing directive-free file still selects legacy behavior. Imported primary modules without a directive also use legacy behavior independently. Textually included files inherit the including module's language version and do not declare their own `#language` or `module` header.

The source directive is authoritative. `.sha.json` does not store or override the Slang language version, because imported modules may not have a Shader Studio config and must remain self-describing when used with `slangc` or another Slang host.
...
- Pinned modern headers for newly generated Slang files.
- Modern language/module header preservation.
- Included fragments inherit their primary module's language version without duplicate headers.
...
- Newly generated `.slang` files declare the pinned modern language version and module name.

...
| `<name>` | — | `ShaderStudioChannel2D` / `…Cube` / `…3D` | Input channel object: sampling, size, playback time, loaded state, native texture and sampler |
...
uses its `inputs` field. See [Channels](channels.md) for configuring slots.

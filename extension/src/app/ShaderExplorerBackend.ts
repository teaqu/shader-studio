...
export class ShaderExplorerBackend {
  ...
  getHtmlForWebview(): string {
    ...
    let processedHtml = rawHtml.replace(
      ...
      (_, attr, file) => {
        ...
        const uri = this.webview.asWebviewUri(vscode.Uri.file(filePath));
    ...
    try {
      ...
      const assetMeta = [
        ...
        const assetUri = this.webview.asWebviewUri(vscode.Uri.file(assetPath)).toString();
    ...
    if (cspMeta) {
      ...
      const styleSrc = `style-src 'self' 'unsafe-inline' ${this.webview.cspSource}`;
      const imgSrc = `img-src 'self' data: blob: ${this.webview.cspSource}`;
      const mediaSrc = `media-src 'self' blob: ${this.webview.cspSource}`;
      ...
      updatedCsp = this.ensureCspToken(updatedCsp, 'script-src', this.webview.cspSource);
      ...
      if (hasSlangAssets) {
        ...
        updatedCsp = this.replaceCspDirective(
          updatedCsp,
          'worker-src',
          `worker-src ${this.webview.cspSource} blob:`,
        );
        updatedCsp = this.replaceCspDirective(
          updatedCsp,
          'connect-src',
          `connect-src ${this.webview.cspSource} blob:`,
        );
      }
      ...
      let newCsp = `default-src 'self'; script-src 'self' 'unsafe-inline' ${this.webview.cspSource}; style-src 'self' 'unsafe-inline' ${this.webview.cspSource}; img-src 'self' data: blob: ${this.webview.cspSource}; media-src 'self' blob: ${this.webview.cspSource}; font-src 'self'; connect-src 'self'`;
      if (hasSlangAssets) {
  ...
  private async findAllShaders(): Promise<ShaderExplorerFile[]> {
    ...
    for (const folder of workspaceFolders) {
      ...
      for (const file of shaderFiles) {
        ...
        // Skip shaders without mainImage entry point
        if (!this.hasMainImage(file.fsPath)) {
          continue;
        }

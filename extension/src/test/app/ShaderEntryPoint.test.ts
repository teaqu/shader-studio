...
suite('ShaderEntryPoint', () => {
  suite('definesMainImage', () => {
    ...
    test('accepts WGSL entry points with generic return types and attributes', () => {
      assert.strictEqual(definesMainImage('fn mainImage(p: vec2f) -> vec4<f32> { return vec4f(1); }'), true);
      assert.strictEqual(definesMainImage('fn mainImage(p: vec2f) -> @location(0) vec4f { return vec4f(1); }'), true);
      assert.strictEqual(definesMainImage('// fn mainImage(p: vec2f) -> vec4f {}'), false);
    });

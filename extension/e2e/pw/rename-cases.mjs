...
const sources = {
  slang: {
    declaration: 'float tone(float value) { return value * 0.5; }\n',
    pass: 'float4 mainImage(float2 coord) { return float4(tone(coord.x)); }\n',
  },
  wgsl: {
    declaration: 'fn tone(value: f32) -> f32 { return value * 0.5; }\n',
    pass: 'fn mainImage(coord: vec2f) -> vec4f { return vec4f(tone(coord.x)); }\n',
  },
};

export function registerRenameTests(language) {
  const label = language === 'wgsl' ? 'WGSL' : 'Slang';
  test.use({ vscodeKey: `${language}-rename` });

  for (const common of [false, true]) {
    test(`renames a ${label} ${common ? 'Common' : 'local'} helper through F2 and saves every reference`, async ({ vscode }) => {
      const basename = `${language}-rename-${common ? 'common' : 'local'}`;
      const shaderPath = join(workspacePath, `${basename}.${language}`);
      const configPath = join(workspacePath, `${basename}.sha.json`);
      const commonPath = join(workspacePath, `${basename}.common.${language}`);
      const { declaration, pass } = sources[language];
      const source = common ? pass : declaration + pass;
      try {
        writeFileSync(shaderPath, source);
        if (common) {
          writeFileSync(commonPath, declaration);
        }
        writeFileSync(configPath, JSON.stringify({ version: '1.0', passes: {
          ...(common ? { common: { path: `${basename}.common.${language}` } } : {}),
          Image: { inputs: {} },
        } }));
        ...
        if (common) {
          expect(readFileSync(commonPath, 'utf8')).toBe(declaration.replace('tone', 'curve'));
        }
      } finally {

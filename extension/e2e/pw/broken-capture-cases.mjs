...
/**
 * Slang and WGSL plan a capture from the whole module, so a statement they
 * cannot parse used to fail every variable at every line. The plan now runs
 * against a copy cut above the break, which still holds the values declared
 * before it.
 */
export function registerBrokenCaptureTests(language) {
  const label = language === 'wgsl' ? 'WGSL' : 'Slang';
  const shaderPath = join(workspacePath, `broken-capture.${language}`);

  test.use({ vscodeKey: `${language}-broken-capture` });

  test.describe(`${label} capture around a broken statement`, () => {
    let state;

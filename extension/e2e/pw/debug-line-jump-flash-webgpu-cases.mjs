...
/**
 * The Slang and WGSL mirror of debug-line-jump-flash.e2e.mjs: jumping the cursor
 * between two debugged lines must go straight from one line's visualization
 * to the other without flashing the whole shader.
 *
 * Plan-based line jumps compile exactly one debug plan per move, so there is no
 ...
 * compile would hold the whole shader on screen for the duration of a
 * compile, which dense sampling cannot miss. Only the real host shows this:
 * it needs debug mode, cursor moves, and genuine WebGPU compiles.
 */
export function registerLineJumpFlashTests(language) {
  const label = language === 'wgsl' ? 'WGSL' : 'Slang';
  const shaderPath = join(workspacePath, `line-jump-flash.${language}`);

  test.use({ vscodeKey: `debug-line-jump-flash-${language}` });

  test.describe(`${label} inline rendering across line jumps @gpu`, () => {
    ...
    // script-pause spec reads its preview.
    ...
    test('goes from one line to the next without showing the whole shader', {
      ...
      await expect.poll(async () => {
        ...
        message: `the untouched ${label} shader never rendered green`, timeout: 120_000,
      }).toBe('green');
      ...
      const settleFor = (oneBasedLine, label, minAvg, maxAvg) => expect.poll(async () => {
        ...
        if (!info.includes(`L${oneBasedLine}`) || !pixel) {
          return 'waiting for the panel';
        }
        if (!isGrey(pixel)) {
          return 'waiting for inline rendering';
        }
      ...
      expect(
        ...
        `a jump showed the whole ${label} shader`,
      ).toEqual([]);

...
describe('performancePanelStore', () => {
  ...
  it('should not restore from localStorage on creation (deferred restore)', async () => {
    localStorage.setItem('shader-studio-performance-panel-state', JSON.stringify({
      isVisible: true,
    }));
    const store = await importStore();
    const state = get(store);
  ...
  it('restoreFromStorage should handle invalid localStorage gracefully', async () => {
    localStorage.setItem('shader-studio-performance-panel-state', 'not-json');
    const store = await importStore();
    store.restoreFromStorage();
  ...
  it('toggle should persist to localStorage', async () => {
    const store = await importStore();
    store.toggle();
    const stored = JSON.parse(localStorage.getItem('shader-studio-performance-panel-state')!);

...
describe('ChannelConfigModal', () => {
  ...
  describe('Cubemaps and Volumes Tabs', () => {
    it('should show cubemap configuration when Cubemaps tab is clicked', async () => {
      render(ChannelConfigModal, defaultProps());

      const cubemapsTab = screen.getByRole('tab', { name: 'Cubemaps' });
      await fireEvent.click(cubemapsTab);

      expect(screen.getByText(/Cubemap input uses the CubeA render pass/)).toBeInTheDocument();
    });

    it('should show volume configuration when Volumes tab is clicked', async () => {
      render(ChannelConfigModal, defaultProps());

      const volumesTab = screen.getByRole('tab', { name: 'Volumes' });
      await fireEvent.click(volumesTab);

      expect(screen.getByText(/3D texture data/)).toBeInTheDocument();
    });
  });

...
describe('WebSocketTransport', () => {
    ...
    function createTransport(port: number = 8080): { transport: WebSocketTransport; mockWs: MockWebSocket } {
        (global.window as any).shaderViewConfig = { port };
    ...
    describe('Connection Lifecycle', () => {
        ...
        it('should handle WebSocket constructor throwing', () => {
            (global.window as any).shaderViewConfig = { port: 8080 };
            webSocketSpy.mockImplementationOnce(() => { throw new Error('Connection refused'); });
    ...
    describe('Message Handling', () => {
        ...
        it('should catch and log handler errors without affecting other handlers', () => {
            const { transport, mockWs } = createTransport();
            const errorHandler = vi.fn().mockImplementation(() => { throw new Error('handler boom'); });
        ...
        it('should handle non-Error thrown in handler', () => {
            const { transport, mockWs } = createTransport();
            const errorHandler = vi.fn().mockImplementation(() => { throw 'string error'; });
            ...
            expect(consoleErrorSpy).toHaveBeenCalledWith(
                'WebSocket: Handler error stack:', 'No stack'
            );
        });
    });
    ...
    describe('dispose', () => {
        ...
        it('should handle dispose when no WebSocket exists', () => {
            (global.window as any).shaderViewConfig = { port: 8080 };
            webSocketSpy.mockImplementationOnce(() => { throw new Error('fail'); });
            const transport = new WebSocketTransport();

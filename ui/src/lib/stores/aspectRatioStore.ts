...
const createAspectRatioStore = () => {
    ...
    if (typeof window !== 'undefined') {
        const stored = localStorage.getItem('shader-view-aspect-ratio');
        if (stored) {
            try {
    ...
    return {
        ...
        setMode: (mode: AspectRatioMode) => {
            update(state => {
                ...
                if (typeof window !== 'undefined') {
                    localStorage.setItem('shader-view-aspect-ratio', JSON.stringify(newState));
                }
                return newState;
            });
        },
        reset: () => {
            ...
            set(defaultState);
            if (typeof window !== 'undefined') {
                localStorage.setItem('shader-view-aspect-ratio', JSON.stringify(defaultState));
            }
        }
    };
};

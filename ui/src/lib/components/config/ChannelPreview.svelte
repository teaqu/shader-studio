...
<div class="channel-preview-container">
  ...
  {:else if channelInput.type === "cubemap"}
    ...
    <div class="cubemap-preview">
      ...
      <div class="cubemap-label">{channelInput.source || "CubeA"}</div>
    </div>
  {:else if channelInput.type === "volume"}
    <!-- Volume preview -->
    <div class="volume-preview">
      <div class="volume-cube-icon">
        <div class="vol-layer"></div>
        <div class="vol-layer"></div>
        <div class="vol-layer"></div>
      </div>
...
<style>
  ...
  .vol-layer:nth-child(1) {
    top: 0;
    left: 0;
  }

  .vol-layer:nth-child(2) {
    top: 3px;
    left: 3px;
    opacity: 0.7;
  }

  .vol-layer:nth-child(3) {
    top: 6px;
    left: 6px;
    opacity: 0.5;
  }

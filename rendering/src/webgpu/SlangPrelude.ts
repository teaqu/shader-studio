...
export interface SlangAuthoredSource {
  readonly label: string;
  readonly source: string;
}

/**
 * Finds top-level authored declarations that would redefine generated direct
 * channel globals. This deliberately ignores function bodies: locals may
 * shadow a global in Slang, while top-level declarations make the module
 * invalid before the compiler can provide a useful channel-config diagnosis.
 */
export function findSlangChannelDeclarationCollisions(
  channels: readonly SlangChannelBinding[] | undefined,
  sources: readonly SlangAuthoredSource[],
): string[] {
  const channelNames = new Set(channels?.map(({ key }) => key) ?? []);
  if (channelNames.size === 0) {
    return [];
  }
  const collisions = new Set<string>();
  for (const { label, source } of sources) {
    for (const { name } of findSlangAuthoredDeclarations(source)) {
      if (channelNames.has(name)) {
        collisions.add(`Slang channel "${name}" conflicts with an authored declaration in ${label}. Rename the channel or declaration.`);
      }
    }
  }
  return [...collisions];

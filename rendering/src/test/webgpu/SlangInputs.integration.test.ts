...
describe('native Slang input objects compile to WGSL', () => {
  it.each([
    ...
    'iChannel0.SampleLevel(c, 2.0)',
    'iChannel0.SampleGrad(c, float2(0.1, 0), float2(0, 0.1))',
    'iChannel0.Sample(iChannel0.sampler, c)',
    'iChannel0.SampleLevel(iChannel0.sampler, c, 2)',
    'iChannel0.SampleGrad(iChannel0.sampler, c, float2(0.1, 0), float2(0, 0.1))',
    'sky.Sample(sky.sampler, float3(c, 1))',
    'sky.SampleLevel(sky.sampler, float3(c, 1), 2)',
    'sky.SampleGrad(sky.sampler, float3(c, 1), float3(0.1, 0, 0), float3(0, 0.1, 0))',
    'iChannel0.texture.Sample(iChannel0.sampler, c)',
    'sky.Sample(float3(c, 1))',
    'sky.SampleLevel(float3(c, 1), 1)',
    'sky.SampleGrad(float3(c, 1), float3(0.1, 0, 0), float3(0, 0.1, 0))',
    'float4(float2(iChannel0.size), iChannel0.time, iChannel0.loaded ? 1.0 : 0.0)',
...
float4 mainImage(float2 c) { return read(iChannel0, c) + sky.Sample(float3(c, 1)); }`, { channels });
    expect(result.success, JSON.stringify(result)).toBe(true);
    if (result.success) {
...
void run(uint3 id : SV_DispatchThreadID) { float2 c = float2(id.xy); writeOutput(id.xy, iChannel0.${call}); }`,
...
void run(uint3 id : SV_DispatchThreadID) { writeOutput(id.xy, iChannel0.Sample(float2(id.xy))); }`,
    { channels, passKind: 'compute', hasOutput: true });
    expect(result.success).toBe(false);
  });
  it.each(['sampleIChannel0(c)', 'iCh0.sampler.Sample(c)', 'iChannel0.Sample(iChannel0Sampler, c)', 'float4(iChannelResolution[0], 1)', 'float4(iChannelTime[0])', 'float4(iChannelLoaded[0])', 'missing.Sample(c)', 'sky.Sample(c)', 'iChannel0.Sample(float3(c, 1))'])('rejects removed or missing symbols: %s', (expression) => {
    const result = compiler.compileImagePass(`float4 mainImage(float2 c) { return ${expression}; }`, { channels });
    expect(result.success).toBe(false);
  });
});

  ...
  it("compiles the dispatch-mode gallery image pass", () => {
    ...
    const result = compiler.compileImagePass(`
      float4 mainImage(float2 fragCoord)
      {
        ...
        float4 texel = iChannel0.Sample(localUv);
        float4 count = iChannel1.Sample(localUv);
        float4 workgroups = iChannel2.Sample(localUv);
        float4 storageCover = iChannel3.Sample(localUv);

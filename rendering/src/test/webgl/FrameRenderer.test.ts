...
describe("FrameRenderer", () => {
  ...
  describe("getChannelTimes", () => {
    it("returns Image channel times in binding order, including named channels", () => {
      mockResourceManager.getAudioState.mockReturnValue({ currentTime: 12.5 });
      mockShaderPipeline.getPasses.mockReturnValue([{
        name: 'Image',
        shaderSrc: 'image shader',
        inputs: { music: { type: 'audio', path: 'music.mp3' } },
      }]);

      expect(frameRenderer.getChannelTimes()).toEqual([12.5, 0, 0, 0]);
    });

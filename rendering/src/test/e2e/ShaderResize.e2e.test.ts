...
const programs: Record<ShaderLanguage, ShaderProgram> = {
  ...
  slang: {
    ...
    float4 mainImage(float2 fragCoord) {
      float history = iChannel0.Sample(float2(12.5, 10.5) / iResolution.xy).r;
      float scaledSize = iChannel1.Sample(float2(0.5)).g;
      float fixedSize = iChannel2.Sample(float2(0.5)).b;

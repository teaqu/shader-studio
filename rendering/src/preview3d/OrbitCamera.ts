...
export class OrbitCamera {
  ...
  getPosition(): [number, number, number] {
    ...
    return [this.target[0] + Math.sin(this.yaw) * horizontalDistance, this.target[1] + Math.sin(this.pitch) * this.distance, this.target[2] + Math.cos(this.yaw) * horizontalDistance];
  }
  ...
  orbit(deltaX: number, deltaY: number): void {
    this.yaw -= deltaX * 0.01; this.pitch = clamp(this.pitch + deltaY * 0.01, MIN_PITCH, MAX_PITCH); 
  }
  ...
  pan(deltaX: number, deltaY: number): void {
    ...
    const up = normalizeVector([right[1] * forward[2] - right[2] * forward[1], right[2] * forward[0] - right[0] * forward[2], right[0] * forward[1] - right[1] * forward[0]]);
  ...
  attach(canvas: HTMLCanvasElement): void {
    this.detach(); this.attachedCanvas = canvas;
  ...
  detach(): void {
    ...
    canvas.removeEventListener('pointerdown', this.onPointerDown); canvas.removeEventListener('pointermove', this.onPointerMove); canvas.removeEventListener('pointerup', this.onPointerUp); canvas.removeEventListener('pointercancel', this.onPointerUp); canvas.removeEventListener('wheel', this.onWheel);
    this.attachedCanvas = null; this.dragMode = null;
  }
  ...
  private readonly onPointerDown = (event: PointerEvent): void => {
    ...
    this.dragMode = event.button === 1 || event.shiftKey ? 'pan' : 'orbit'; this.lastPointer = [event.clientX, event.clientY]; this.attachedCanvas?.setPointerCapture?.(event.pointerId); event.preventDefault();
  };
  private readonly onPointerMove = (event: PointerEvent): void => {
    if (!this.inputEnabled || !this.dragMode) {
      return;
    }
    const deltaX = event.clientX - this.lastPointer[0]; const deltaY = event.clientY - this.lastPointer[1]; this.lastPointer = [event.clientX, event.clientY];
  ...
  private readonly onPointerUp = (event: PointerEvent): void => {
    this.dragMode = null; this.attachedCanvas?.releasePointerCapture?.(event.pointerId); 
  };
  private readonly onWheel = (event: WheelEvent): void => {
    if (!this.inputEnabled) {
      return;
    } this.dolly(event.deltaY); event.preventDefault(); 
  };
}

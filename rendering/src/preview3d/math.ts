...
export function createPerspectiveMatrix(fieldOfViewRadians: number, aspect: number, near: number, far: number, clipSpaceDepth: ClipSpaceDepth = 'webgl'): Mat4 {
  ...
  return new Float32Array([f / aspect, 0, 0, 0, 0, f, 0, 0, 0, 0, depthScale, -1, 0, 0, depthOffset, 0]);
}
...
export function createModelMatrix(transform: PreviewObjectTransform): Mat4 {
  const [sx, sy, sz] = transform.scale; const [rx, ry, rz] = transform.rotation; const [tx, ty, tz] = transform.position;
  const sinX = Math.sin(rx); const cosX = Math.cos(rx); const sinY = Math.sin(ry); const cosY = Math.cos(ry); const sinZ = Math.sin(rz); const cosZ = Math.cos(rz);
  return new Float32Array([(cosZ * cosY) * sx, (sinZ * cosY) * sx, -sinY * sx, 0, (cosZ * sinY * sinX - sinZ * cosX) * sy, (sinZ * sinY * sinX + cosZ * cosX) * sy, cosY * sinX * sy, 0, (cosZ * sinY * cosX + sinZ * sinX) * sz, (sinZ * sinY * cosX - cosZ * sinX) * sz, cosY * cosX * sz, 0, tx, ty, tz, 1]);
}

export function createNormalMatrix3(model: Mat4): Float32Array {
  const a00 = model[0]; const a01 = model[4]; const a02 = model[8]; const a10 = model[1]; const a11 = model[5]; const a12 = model[9]; const a20 = model[2]; const a21 = model[6]; const a22 = model[10];
  const c00 = a11 * a22 - a12 * a21; const c01 = a12 * a20 - a10 * a22; const c02 = a10 * a21 - a11 * a20; const c10 = a02 * a21 - a01 * a22; const c11 = a00 * a22 - a02 * a20; const c12 = a01 * a20 - a00 * a21; const c20 = a01 * a12 - a02 * a11; const c21 = a02 * a10 - a00 * a12; const c22 = a00 * a11 - a01 * a10;
...
export function transformPoint(matrix: Mat4, point: Vec3): [number, number, number] {
  const [x, y, z] = point; const w = matrix[3] * x + matrix[7] * y + matrix[11] * z + matrix[15]; const reciprocalW = Math.abs(w) > EPSILON ? 1 / w : 1;
  return [(matrix[0] * x + matrix[4] * y + matrix[8] * z + matrix[12]) * reciprocalW, (matrix[1] * x + matrix[5] * y + matrix[9] * z + matrix[13]) * reciprocalW, (matrix[2] * x + matrix[6] * y + matrix[10] * z + matrix[14]) * reciprocalW];
}

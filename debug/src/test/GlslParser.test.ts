...
describe("GlslParser", () => {
  ...
  describe("buildVariableTypeMap", () => {
    ...
    it("should include declarations whose initializers are changed by macro expansion", () => {
      const lines = [
        "#define gx 15.0",
        "#define gy 10.0",
        "",
      ...
      const functionInfo = GlslParser.findEnclosingFunction(lines, 10);
      const varTypes = GlslParser.buildVariableTypeMap(lines, 10, functionInfo);

      expect(varTypes.get("uGrid")).toBe("vec2");
      expect(varTypes.get("col")).toBe("vec3");
    });
  ...
  describe("buildVariableLineMap", () => {
    ...
    it("should map macro-expanded declarations to their original source lines", () => {
      const lines = [
        "#define gx 15.0",
        "#define gy 10.0",
        "",
      ...
      const funcInfo = GlslParser.findEnclosingFunction(lines, 10);
      const varTypes = GlslParser.buildVariableTypeMap(lines, 10, funcInfo);
      const lineMap = GlslParser.buildVariableLineMap(lines, 10, funcInfo, varTypes);

      expect(lineMap.get('uGrid')).toBe(6);
      expect(lineMap.get('col')).toBe(9);
    });

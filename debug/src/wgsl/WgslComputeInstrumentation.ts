...
export function buildWgslComputeInstrumentation(
  ...
  if (!compute && document.symbols.some(symbol => symbol.kind === "function" && symbol.name === "mainImage")) {
    return undefined;
  }
  ...
  if (hasWorkgroupMemory(tokens)) {
    return "WGSL compute replay does not support workgroup memory.";
  }
  ...
  if (fnIndex < 0) {
    return `WGSL compute entry '${selected.name}' could not be located.`;
  }
  const bodyIndex = findBodyStart(tokens, fnIndex);
  if (bodyIndex < 0) {
    return `WGSL compute entry '${selected.name}' has no body.`;
  }
  ...
  if (!functionSymbol) {
    return `WGSL compute entry '${selected.name}' could not be analysed.`;
  }
...
function parseBuiltinParameters(tokens: readonly WgslToken[], fnIndex: number, bodyIndex: number): BuiltinParameter[] | string {
  ...
  if (open < 0 || open >= bodyIndex) {
    return "WGSL compute entry parameters could not be read.";
  }
  const parameters: BuiltinParameter[] = [];
  let index = open + 1;
  while (index < bodyIndex && tokens[index]?.text !== ")") {
    const attribute = readAttribute(tokens, index);
    if (!attribute || attribute.name !== "builtin") {
      return "WGSL compute replay only supports @builtin entry parameters.";
    }
    ...
    while (cursor < bodyIndex) {
      ...
      if (angleDepth === 0 && (token.text === "," || token.text === ")")) {
        break;
      }
...
function readAttribute(tokens: readonly WgslToken[], start: number): { name: string; values: string[]; end: number; tokens: WgslToken[] } | undefined {
  if (tokens[start]?.kind !== "attribute") {
    return undefined;
  }
...
function computeAttributeEdits(tokens: readonly WgslToken[], fnIndex: number, source: string): DebugSourceEdit[] {
  ...
  for (let index = fnIndex - 1; index >= 0 && tokens[index]?.text !== "}" && tokens[index]?.text !== ";"; index -= 1) {
    ...
    const attribute = readAttribute(tokens, index);
    if (attribute && (attribute.name === "compute" || attribute.name === "workgroup_size")) {
      edits.push(...attributeEdit(attribute.tokens, source));
    }
  }
  return edits;
}
...
function writesConfiguredStorage(tokens: readonly WgslToken[], storageNames: ReadonlySet<string>): boolean {
  ...
  return tokens.some((token, index) => {
    if (!storageNames.has(token.text)) {
      return false;
    }
...
function outputStub(tokens: readonly WgslToken[]): { declaration?: string; error?: string } | undefined {
  if (tokens.some((token, index) => token.text === "fn" && tokens[index + 1]?.text === "writeOutput")) {
    return undefined;
  }
  const arities = new Set<number>();
  for (let index = 0; index < tokens.length; index += 1) {
    if (tokens[index]?.text !== "writeOutput" || tokens[index + 1]?.text !== "(") {
      continue;
    }
    let depth = 0;
    let arity = 1;
  ...
  if (arities.size !== 1) {
    return { error: "WGSL compute replay requires writeOutput calls to use one arity." };
  }
